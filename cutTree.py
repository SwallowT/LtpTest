...
from ltp_standard import ltp_prepro
from s_expr_parse import tuple2tree
from treelib import Tree, Node
from s_expr_parse import treelib2s_expr


verb_keys = ['包括', '属于', '分为', '分成', '例如', '有：', '如下', ]  # '可识别为', '是', '如', '涉及'
SUB = ['AGT', 'EXP']
OBJ = ['PAT', 'CONT', 'DATV', 'LINK']
SCENE = ['TOOL', 'MATL', 'MANN', 'SCO', 'REAS', 'TIME', 'LOC', 'MEAS', 'STAT']  # , 'FEAT.txt'
dump_tag = SCENE + ['mRELA', 'mDEPD']  # 删除情景依存关系
merge_tag = ['FEAT', 'eCOO']
...
def Rule_template(old_tree, condition, isprint=False, extra_func=None):
    ...
    for node in old_tree.all_nodes():  # 广度优先遍历
        ...
        if node.identifier not in dumped_ids:
            ...
            for child_id in children:
                ...
                if condition(child.tag) and child.identifier not in dumped_ids:
    ...
    if isprint and len(dumped_ids) > 0:
        ...
        print(" ".join([old_tree.get_node(nd).data for nd in sorted_dumped_ids]))
...
def cut_tree(ltp_tree):
    ...
    tree_temp = Rule_template(tree_temp, condition=lambda tag: tag.strip('rd') in merge_tag,
                              extra_func=paste_subtree_2_parent)  # 合并依存关系
    tree_temp = Rule_template(tree_temp, condition=lambda tag: tag.strip('rd') in dump_tag)  # 删除该删除的依存关系tag
    # tree_temp = Rule_template(tree_temp, condition=lambda tag: tag == 'm')
...
if __name__ == "__main__":
    ...
    trees = [tuple2tree(sdp, seg) for sdp, seg in zip(sdps, segments)]
    with open('data/cutted_trees_sexpr.txt', 'w', encoding='utf-8') as f:
        for tree in trees:
